...
class DeliveryChargeRepository
{
    ...
    public function update($request, $company)
    {
        ...
        try {
            ...
            if (isset($request->days_status) && !empty($request->days_status) && config('setting.other.select_shipping_provider') == 'shipping_company' && in_array('schedule', $company->delivery_time_types ?? [])) {
                $this->updateDeliveryTimes($company, $request->days_status, $request->is_full_day, $request->availability);
            }
    ...
    private function updateDeliveryTimes($company, $days_status, $is_full_day, $availability)
    {
        // START Edit Work Times Over Weeks
        $deletedWorkTimes = $this->syncRelationModel($company, 'deliveryTimes', 'day_code', $days_status);
        foreach ($days_status as $k => $dayCode) {
            if (array_key_exists($dayCode, $is_full_day)) {
                if ($is_full_day[$dayCode] == '1') {
                    ...
                    $company->deliveryTimes()->updateOrCreate(['day_code' => $dayCode], $availabilityArray);
                } else {
                    $works = [
                        'day_code' => $dayCode,
                        'status' => true,
                    ...
                    $company->deliveryTimes()->updateOrCreate(['day_code' => $dayCode], $works);
                }
            }
        }

        if (!empty($deletedWorkTimes['deleted'])) {
            $company->deliveryTimes()->whereIn('day_code', $deletedWorkTimes['deleted'])->delete();
        }
        return true;
