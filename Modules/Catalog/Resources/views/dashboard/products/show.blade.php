                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tr>
                                ...
                                <td>{{ $product->status == 1 ? __('apps::dashboard.general.active') : __('apps::dashboard.general.not_active') }}
                                </td>
                            </tr>
                            <tr>
                                <td class="bold">{{ __('catalog::dashboard.products.form.featured') }}</td>
                                <td>{{ $product->featured == 1 ? __('apps::dashboard.general.yes_btn') : __('apps::dashboard.general.no_btn') }}
                                </td>
                            </tr>
                            <tr>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            @foreach ($product->translations as $k => $item)
                                <tr>
                                    <td class="bold">
                                        {{ __('catalog::dashboard.products.form.title') . '-' . $item->locale }}</td>
                                    <td>{{ $item->title }}</td>
                                </tr>
                                <tr>
                                    <td class="bold">
                                        {{ __('catalog::dashboard.products.form.description') . '-' . $item->locale }}</td>
                                    <td>
                                        <a href="javascript:;" data-toggle="modal"
                                            data-target="#prdModal-{{ $k . '-' . $item->locale . '-desc' }}">{{ limitString(strip_tags($item->description)) }}</a>
                                        @include('catalog::dashboard.products._show_modal', [
                                            'index' => $k . '-' . $item->locale . '-desc',
                                            'modalTitle' =>
                                ...
                                <tr>
                                    <td class="bold">
                                        {{ __('catalog::dashboard.products.form.short_description') . '-' . $item->locale }}
                                    </td>
                                    <td>
                                        <a href="javascript:;" data-toggle="modal"
                                            data-target="#prdModal-{{ $k . '-' . $item->locale . '-short_desc' }}">{{ limitString(strip_tags($item->short_description)) }}</a>
                                        @include('catalog::dashboard.products._show_modal', [
                                            'index' => $k . '-' . $item->locale . '-short_desc',
                                            'modalTitle' =>
                        ...
                        <ul>
                            @foreach ($product->addOns as $k => $item)
                                <li>
                                    ...
                                    @include('catalog::dashboard.products._show_addons_modal', [
                                        'item' => $item,
                                    ])
                                </li>
                            @endforeach
                    ...
                    @foreach ($product->images as $k => $item)
                        <div class="col-md-2">
                            <div style="height: 190px; width: 100%;">
                                <img class="img-thumbnail" src="{{ url('uploads/products/' . $item->image) }}"
                                    alt="Product Image" style="height: inherit; width: inherit;">
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                @foreach ($product->variants as $k => $item)
                    <div class="row">
                        <div class="col-md-8">
                            <div class="table-responsive">
                                <table class="table table-bordered">
                                    ...
                                    <tr>
                                        <td class="bold">{{ __('catalog::dashboard.products.form.status') }}</td>
                                        <td>{{ $item->status == 1 ? __('apps::dashboard.general.active') : __('apps::dashboard.general.not_active') }}
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="bold">{{ __('catalog::dashboard.products.form.created_at') }}</td>
                        ...
                        @if ($item->image)
                            <div class="col-md-4">
                                <div style="height: 190px; width: 100%;">
                                    <img class="img-thumbnail" src="{{ url($item->image) }}" alt="Product Image"
                                        style="height: inherit; width: inherit;">
                                </div>
                            </div>
                        @endif
